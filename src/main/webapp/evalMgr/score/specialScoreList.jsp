...
<%
	...
	StringBuffer personListStr=new StringBuffer();
	if(requestMap.getString("mode").equals("personList")){
		...
		if(!personList.isEmpty()){
			for(int i=0;i<personList.keySize();i++){	
				personListStr.append("<tr bgcolor='#FFFFFF'>");
				personListStr.append("<td class='tableline11'><div align='center'>"+personList.getString("stu_cnt",i)+"</div></td>");
				personListStr.append("<td class='tableline11'><div align='center'>"+personList.getString("eduno",i)+"</div></td>");
				personListStr.append("<td class='tableline11'><div align='center'>"+personList.getString("name",i)+"</div></td>");
				personListStr.append("<td class='tableline11'><div align='center'>"+personList.getString("resno",i)+"</div></td>");
				personListStr.append("<td class='tableline11'><div align='center'>"+personList.getString("deptnm",i)+"</div></td>");
				personListStr.append("<td class='tableline11'><div align='center'>"+personList.getString("jiknm",i)+"</div></td>");
				personListStr.append("<td class='tableline11'><div align='center'>"+personList.getString("cur_avlcount",i)+"</td>");
				personListStr.append("<td class='tableline21'><div align='center'>");
				personListStr.append("<input type='hidden' name='h_userno[]' value="+personList.getString("userno",i)+">");
				personListStr.append("<input type='text' name='h_point[]' value='"+personList.getString("avlcount",i)+"' size=5></div></td>");
				personListStr.append("</tr>");
			}
		}else{
			personListStr.append("<tr bgcolor='#FFFFFF'>");
			if(requestMap.getString("sessClass").equals("7")) personListStr.append("<td align='center' colspan='8' height='30'>지정된 과목이 없습니다</td>");
			else personListStr.append("<td align='center' colspan='8' height='30'>검색된 내역이 없습니다</td>");
			personListStr.append("</tr>");
		}
	}
%>
	...
	function submitQa(totpoint) {
		...
		for(var i=0;i<document.forms["pform"].elements["h_point[]"].length;i++){
			if (Number(pointObj[i].value) > Number(totpoint)){				
				var j=i+1;
				alert(j+" 번째 입력하신 점수가 총점("+totpoint+") 보다 큽니다. 저장할 수 없습니다.");
				return;
			}
		}
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="E5E5E5">
							<tr>
								<td colspan="10" bgcolor="FFFFFF">
									<input type="text" name="all_jum" size="3">점 
									<input type="button" value="전체점수적용" onClick="ins_jum(document.forms['pform'].all_jum.value)" class="boardbtn1">	
								</td>
							</tr>			
							<tr bgcolor="#375694"> 
							...
							<tr bgcolor="#5071B4"  height='28' > 
								...
								<td  align='center' class="tableline11 white"><div align="center"><strong>점수수정</strong></div></td>
							</tr>
							<%=personListStr.toString() %>							
						</table>					 								
					 <%}%>
				</td>
			</tr>

...
<%
	...
	if(listMap.keySize("rdept") > 0){		
		...
		sbListHtml.append("	<td style=\"text-align:center\"></td>");
		sbListHtml.append("	<td style=\"text-align:center\">&nbsp;<b>" + listMapTotal.getString("rdeptsubCnt", 0) + "개 부서기관 </b></td>");
		...
		for(int i=0; i < listMap.keySize("rdept"); i++){
			...
			sbListHtml.append("	<td style=\"text-align: left\">&nbsp;" + listMap.getString("rdeptsub", i) + "</td>");	
...
function fnSearch(){
	if(NChecker($("pform"))){
		...
		$("mode").value = "cyberDetailDeptRgister";
...
function fnExcel(){
	if(NChecker($("pform"))){
		...
		$("mode").value = "cyberDetailDeptRgisterExcel";
		pform.action = "/statisMgr/stats.do";
		pform.submit();
	}
}
    ...
    <tr>
        ...
        <td colspan="2" valign="top" class="leftMenuBg">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="contentsSubTitleTable">			
				<tr>
					<td height="20">
						<img src="/images/bullet003.gif"  align="bottom">&nbsp;<strong>사이버 세부기간별 등록현황 </strong>
					</td>
				</tr>
			</table>
			<!--[e] subTitle -->
					...
					<td>
							...
							<tr>
								...
								<td>
									...
									<select id="rgrayn" name="rgrayn">
									     <option value="N" <%= "N".equals(rgrayn) ? "selected":"" %>>전체</option>
						...
						<table class="datah01">
							...
							<tr>
								<th>기 관 명</th>
								<th>부 서 명</th>
								<th><%=grseq.substring(4)%>기 등록인원</th>
								<th>비 고</th>
							</tr>
							</thead>

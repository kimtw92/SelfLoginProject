...
<%
	...
	// 기관셀렉트 박스
	StringBuffer sbDeptHtml = new StringBuffer();
	
	// 현재 권한이 기관담당자가 아닐때만 기관 셀렉트 박스가 생성된다.
	if(!memberInfo.getSessCurrentAuth().equals("3")){
		
		String selected = "";
		
		DataMap deptMap = (DataMap)request.getAttribute("DEPT_DATA");
		if(deptMap == null) deptMap = new DataMap();
		deptMap.setNullToInitialize(true);
		
		sbDeptHtml.append("<select name=\"searchDept\" id=\"searchDept\" >");
		sbDeptHtml.append("	<option value=\"\">**기관선택**</option>");
		
		if(deptMap.keySize("dept") > 0){		
			for(int i=0; i < deptMap.keySize("dept"); i++){
				
				if(deptMap.getString("dept",i).equals(searchDept)){
					selected = "selected";
				}else{
					selected = "";
				}
				
				sbDeptHtml.append("	<option value=\"" + deptMap.getString("dept", i) + "\" " + selected + ">" + deptMap.getString("deptnm", i) + "</option>");
			}
		}
	}else{
		searchDept = memberInfo.getSessDept();
		sbDeptHtml.append("<input type=\"hidden\" name=\"searchDept\" id=\"searchDept\" value=\"" + searchDept + "\">");
	}
...
function fnPrint(){
	if(NChecker($("pform"))){
		popAI("http://<%= Constants.AIREPORT_URL %>/report/report_5.jsp?p_startmonth=" + $F("yearMonthFrom") + "&p_endmonth=" + $F("yearMonthTo") + "&p_dept=<%= searchDept %>" );
	}
}
    ...
    <tr>
        ...
        <td colspan="2" valign="top" class="leftMenuBg">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="contentsSubTitleTable">			
				<tr>
					<td height="20">
						<img src="/images/bullet003.gif"  align="bottom">&nbsp;<strong>과정별 통계</strong>
					</td>
				</tr>
			</table>
			<!--[e] subTitle -->
				...
				<tr>
					<td>
						...
						<table class="search01">
                            <tr <%= memberInfo.getSessCurrentAuth().equals("3") ? " style='display:none' " : "" %> >

...
<%
	...
	if(loginDisplayYn.equals("Y")){
		...
		PaperService service = context.getBean(PaperService.class);
		
		DataMap paperCountData = null;		// 현재 쪽지 갯수
		paperCountData = service.paperNewCount(memberInfo.getSessNo());	
...
if(NChecker(document.loginForm)){
			...
			if ($("security").checked == true){
				...
				//loginForm.action = "http://<%=request.getServerName()%>/homepage/login.do?mode=loginChk";
				//url = "/homepage/login.do";
				//alert(url);
			}
...
<form id="loginForm" name="loginForm" method="post">
		...
		<div class="login" <%= loginDisplayYn.equals("Y") ? "style=\"display:none\"":"" %>>
		  <div class="in_check">
			<input type="checkbox" class="chk" id="security" name="security" checked="checked"/>

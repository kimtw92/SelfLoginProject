...
<title>::지번검색::</title>
<link type="text/css" href="http://hrd.incheon.go.kr/newAddr/style.css" rel="stylesheet" />
<script src="http://hrd.incheon.go.kr/newAddr/common.js"></script>
 ...
 */
   ...
   function changeAreaList(idx, obj){	
		...
		var url = "/newAddr/AjaxRequestXML.jsp?getUrl=http://www.juso.go.kr/getAreaCode.do?" + escape(createParameter());
	...
	function normalSearch(currentPage){
		...
		url = "/newAddr/AjaxRequestXML.jsp?getUrl="+escape("http://www.juso.go.kr/link/search.do?extend=false&mode=jibun_search&searchType=location_jibun&topTab=1&engineCtpNm="+encodeURI(cityText)+"&engineSigNm="+encodeURI(countyText)+"&engineEmdNm="+encodeURI(townText)+"&engineLiNm="+encodeURI(riText)+"&engineBdMaSn="+encodeURI(form.bun1.value)+"&engineBdSbSn="+encodeURI(form.bun2.value)+"&engineMtYn="+encodeURI(form.san1.value)+"&currentPage="+currentPage);
	...
	function normalSearch2(currentPage){
		...
		url = "/newAddr/AjaxRequestXML.jsp?getUrl="+escape("http://www.juso.go.kr/link/search.do?extend=false&mode=jibun_search&searchType=location_jibun&topTab=1&engineCtpNm="+encodeURI(form.engineCtpNm.value)+"&engineSigNm="+encodeURI(form.engineSigNm.value)+"&engineEmdNm="+encodeURI(form.engineEmdNm.value)+"&engineLiNm="+encodeURI(form.engineLiNm.value)+"&engineBdMaSn="+form.engineBdMaSn.value+"&engineBdSbSn="+form.engineBdSbSn.value+"&engineMtYn="+form.engineMtYn.value+"&currentPage="+currentPage);
	...
	*/
	 ...
	 function setOpenerValue(newaddr){
		...
		*/
		 ...
		 if(post.indexOf("-") != -1 ) {
			 postTemp = post.split("-");
			 window.opener.document.getElementById("newHomePost1").value = postTemp[0];
			 window.opener.document.getElementById("newHomePost2").value  = postTemp[1];
		 }
		 window.close();
	 }
	 ...
	 function selectSearch(mode) {
		 if (mode == 'road'){
				document.location.href = "/newAddr/road.jsp";
			}else if(mode =='jibun'){
				document.location.href = "/newAddr/jibun.jsp";
			}else if(mode=='sangho'){
				document.location.href = "/newAddr/sangho.jsp";
			}		
	}

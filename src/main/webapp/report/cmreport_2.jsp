...
class ReportHandler{
	...
	private void getScriptFromtitle(boolean bIsFirst, boolean bIsPrint) throws Exception{
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			...
			//데이터 변수 할당
			sheet10_C2= p_s_date_Param.substring(0,4) + "." + p_s_date_Param.substring(4,6) + "." + p_s_date_Param.substring(6,8) + " ~ "  + p_e_date_Param.substring(0,4) + "." + p_e_date_Param.substring(4,6) + "." + p_e_date_Param.substring(6,8);
		}   //end if(bIsFirst)
	...
	private void setParam(){
		p_s_date_Param=toKor(GetB(request.getParameter("p_s_date")));
		p_e_date_Param=toKor(GetB(request.getParameter("p_e_date")));
	}
	...
	private void initSQLConst(){
		sql_LIST = "select no, ";
		sql_LIST = sql_LIST + "(select name from CMTB_member where userno = a.userno) as name, ";
		sql_LIST = sql_LIST + "(select gadminnm from CMTB_gadmin where gadmin = a.gadmin) as gadminnm ";
		sql_LIST = sql_LIST + ",gadmin, ";
		sql_LIST = sql_LIST + "(select deptnm from CMTB_dept where dept = a.dept) as deptnm, ";
		...
		sql_LIST = sql_LIST + "(select name from CMTB_member where userno = a.luserno) as lname ";
		sql_LIST = sql_LIST + "from CMTB_GADMIN_HISTORY a  ";
		sql_LIST = sql_LIST + "where  ";
		sql_LIST = sql_LIST + "cdate BETWEEN to_date( '" + p_s_date_Param + "','YYYYMMDD') AND to_date( '" + p_e_date_Param + "','YYYYMMDD')  ";

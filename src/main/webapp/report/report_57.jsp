...
class ReportHandler{
	...
	private void writeScriptchart( ) throws Exception{
		...
		out.println("^CHART(" + (nXCurPos+2) + "," + (nYCurPos+43) + ",528,236,report_57!100.cht)\r");
	...
	private void initSQLConst(){
		sql_A = "select (SELECT GRCODENM FROM TB_GRCODE WHERe GRCODE='" + p_grcode_Param + "') || ' ' ||  (select subjnm from tb_subj where subj='" + p_subj_Param + "')  grcodenm ";
		sql_A = sql_A + " ";
		sql_A = sql_A + "from dual";
		sql_L1 = "SELECT substr(sr.grseq,1,4)||'년 ' || substr(sr.grseq,5,2)||'기' as grseq, round(avg(decode(sr.subj,'GUN0000001',sr.avcourse,sr.paccept)/sj.totpoint*100),2) pacceptavg  ";
		sql_L1 = sql_L1 + "FROM tb_subjresult sr , tb_subjseq sj WHERE sj.totpoint > 0 and sr.grcode=sj.grcode  ";
		sql_L1 = sql_L1 + "and sr.grseq=sj.grseq and sr.subj=sj.subj and sr.grcode='" + p_grcode_Param + "' and sr.subj='" + p_subj_Param + "' and sr.grseq between '" + p_startyear_Param + "'||'00' and '" + p_endyear_Param + "' ||'99'  GROUP BY sr.grseq order by sr.grseq";
		sql_L2 = "SELECT substr(sr.grseq,1,4) ||'년' as gryear, ";
		sql_L2 = sql_L2 + "round(avg(decode(sr.subj,'GUN0000001',sr.avcourse,sr.paccept)/sj.totpoint*100),2) pacceptavg  ";
		sql_L2 = sql_L2 + "FROM tb_subjresult sr, tb_subjseq sj WHERE sj.totpoint > 0 and sr.grcode=sj.grcode and sr.grseq=sj.grseq  ";
		sql_L2 = sql_L2 + "and sr.subj=sj.subj and sr.grcode='" + p_grcode_Param + "' and sr.subj='" + p_subj_Param + "' and sr.grseq between '" + p_startyear_Param + "'||'00' and '" + p_endyear_Param + "' ||'99' GROUP BY substr(sr.grseq,1,4) order by substr(sr.grseq,1,4)";
	}
	...
	private void getChartDataAI_CHART_OBJ2( ) throws Exception{
		...
		//계열(Series) 필드 설정
		AI_CHART_OBJ2="\t";
		AI_CHART_OBJ2=AI_CHART_OBJ2 + "PACCEPTAVG\r\n";
		...
		while(L1.next()){   //데이터 루프문
			...
			//X축 레이블 필드 설정
			AI_CHART_OBJ2=AI_CHART_OBJ2 + L1_GRSEQ + "\t";

			//데이터 설정
			AI_CHART_OBJ2=AI_CHART_OBJ2 + L1_PACCEPTAVG + "\r\n";
		}   //while end
	}

...
class ReportHandler{
	...
	private void getScriptFromloop2(boolean bIsPrint) throws Exception{
		...
		while(L2.next()){   //자동으로 생성되는 반복 밴드의 루프문
			...
			sheet30_G5=L2_SEXNM;
			sheet30_H5=L2_AGE;
			sheet30_I5=Double.parseDouble(Get0(L2_PACCEPT));
	...
	private void writeScriptloop2( ) throws Exception{
		...
		out.println("^CELLLINE("+(nXCurPos+427)+","+(nYCurPos)+",36,20,0,RGB[0,0,0],2)\r");
		out.println("^CELLLINE("+(nXCurPos+427)+","+(nYCurPos)+",36,20,0,RGB[0,0,0],3)\r");
		out.print("^PRINT("+(nXCurPos+427)+","+(nYCurPos)+",36,1,10,1/0/2,돋움체,RGB[0,0,0], , , ,NONE,20,1)");
	...
	private void initSQLConst(){
		...
		sql_L2 = sql_L2 + "gr.eduno, to_char(round(gr.paccept,2)) as paccept, jk.jiknm, dt.deptnm, decode(substr(gr.rresno, 7,1),'1','남', '2', '여') sexnm  ";
	...
	private void setFieldVariableFromL2() throws Exception{
		...
		L2_PACCEPT=GetB(L2.getString(6));
	...
	private void initConstVarient(){
		...
		sheet30_L5=1;
		sheet30_B4="계";
		sheet30_A6="※ 성적이 동점인 경우  1순위 근태평가 성적이 우수한 자, 2순위 강사평가 성적이 우수한 자    3순위 자치활동에 공이 많은자, 4순위 연구과제평가 성적이 우수한 자, 5순위 연령이 많은자 순으로 결정";
	}

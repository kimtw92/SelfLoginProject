...
class ReportHandler{
	...
	private java.sql.Statement stat_qry_grcode;		//ResultSet qry_grcode 객체를 생성하기 위한 Statement 객체
	private ResultSet qry_grcode;		//ResultSet qry_grcode 객체
	...
	private String sql_qry_grcode;		//ResultSet qry_grcode 객체를 생성하기 위한 쿼리문 저장

	//-----쿼리 필드 변수 선언-----
	//ResultSet master 객체에서 필드 값을 받는 쿼리 필드 변수
	private String master_QUESTION_NO;
	...
	private String detail_ANSWER_NO;
	private String detail_ANSWER;
	private double detail_ANSWER_KIND;

	//ResultSet qry_grcode 객체에서 필드 값을 받는 쿼리 필드 변수
	private String qry_grcode_GRCODENM;
	...
	void service(JspWriter out) throws Exception{
		try{    //try 구문
			...
			try{    //finally try 구문
				//DataSet Close
				if(qry_grcode!=null) qry_grcode.close();
				if(stat_qry_grcode!=null) stat_qry_grcode.close();
	...
	private void executeSheet10( ) throws Exception{
		//쉬트 초기화 함수 호출
		initSheetVariant(41, 98, 546, 787, true);
		...
		getScriptFromVoidBand104(true, true);    //Void 밴드 호출
	...
	private void getScriptFromVoidBand104(boolean bIsFirst, boolean bIsPrint) throws Exception{
		//지역 변수 선언 및 초기화
		int nBandHeight=46;    //밴드 출력 높이

		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			//DataSet 객체 생성
			//ResultSet qry_grcode 객체의 생성
			if(qry_grcode==null){
				qry_grcode = stat_qry_grcode.executeQuery(sql_qry_grcode);
			}else{
				qry_grcode.beforeFirst();
			}

			//데이터 fetch
			if(qry_grcode.next()){
				setFieldVariableFromqry_grcode();	//ResultSet qry_grcode에서 필드 값 설정
			}else{
				setFieldInitFromqry_grcode();	//ResultSet qry_grcode의 필드 값 초기화
			}
			...
			//데이터 변수 할당
			sheet10_C2=qry_grcode_GRCODENM;
		}    //end if(bIsFirst)
	...
	private void getScriptFromdetail(boolean bIsPrint) throws Exception{
		...
		while(detail.next()){   //자동으로 생성되는 반복 밴드의 루프문
			...
			//데이터 변수 할당
			sheet10_B4=detail_ANSWER_NO;
			sheet10_C4=detail_ANSWER;
	...
	private void getScriptFrommaster(boolean bIsPrint) throws Exception{
		...
		//폰트 사이즈 : 11
		//각 문자별 폭을 할당할 배열 선언
		int[] charWidth = {7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,13};
		...
		while(master.next()){   //자동으로 생성되는 반복 밴드의 루프문
			...
			//데이터 변수 할당
			sheet10_A3=master_QUESTION_NO;
			sheet10_B3=master_QUESTION;

			//Auto Size Code
			nAutoHeight_master[0]=0;
			nAutoHeight_master[0]=getPrintHeight(sheet10_B3, charWidth, 11, 465, 20, nAutoHeight_master[0]);
	...
	private void writeScriptheader( ) throws Exception{
		//스크립트문 생성
		out.print("^PRINT(41,55,505,1,16,0/,굴림체,RGB[0,0,0],BOLD, , ,NONE,43,1)");
		out.println(sheet10_A1+"\r");
	}

	//----VoidBand104 스크립트 문 출력 함수----
	private void writeScriptVoidBand104( ) throws Exception{
		//스크립트문 생성
		out.print("^PRINT("+(nXCurPos+80)+","+(nYCurPos)+",425,2,11,0/,굴림체,RGB[0,0,0],BOLD, , ,NONE,46,1)");
	...
	private void writeScriptdetail( ) throws Exception{
		//스크립트문 생성
		out.print("^PRINT("+(nXCurPos+40)+","+(nYCurPos)+",40,1,11,0/,굴림체,RGB[0,0,0],BOLD, , ,NONE,20,1)");
		out.println(sheet10_B4+"\r");
		out.print("^PRINT("+(nXCurPos+80)+","+(nYCurPos)+",425,0,11,0/,굴림체,RGB[0,0,0],BOLD, , ,NONE,20,1)");
	...
	private void writeScriptmaster( ) throws Exception{
		//스크립트문 생성
		out.print("^PRINT("+(nXCurPos)+","+(nYCurPos)+",40,1,11,0/,굴림체,RGB[0,0,0],BOLD, , ,NONE,"+(nAutoHeight_master[0])+",0)");
		out.println(sheet10_A3+"\r");
		out.print("^PRINT("+(nXCurPos+40)+","+(nYCurPos)+",465,0,11,0/,굴림체,RGB[0,0,0],BOLD, , ,XL,"+(nAutoHeight_master[0])+",0)");
	...
	private void createConnAndStateObject() throws Exception{
		...
		stat_qry_grcode=conn_DUNET.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
	}

	//-----SQL문 변수 초기화 함수-----
	private void initSQLConst(){
		sql_master = "SELECT to_char(a.question_no) || '.' as question_no, a.question as question ";
		sql_master = sql_master + "FROM  TB_GRINQ_QUESTION_SET a, TB_GRINQ_SAMP_SET b ";
		sql_master = sql_master + "WHERE  a.title_no = " + p_title_no_Param + " AND a.set_no = " + p_set_no_Param + " AND a.question_checked_no = 0   ";
		sql_master = sql_master + "AND a.samp_checked_no = 0  AND a.title_no=b.title_no AND a.question_no=b.question_no ";
		sql_master = sql_master + "AND a.set_no=b.set_no  ";
		sql_master = sql_master + "AND answer_kind in (1,2) ";
		sql_master = sql_master + "GROUP BY a.QUESTION_NO,a.QUESTION ";
		sql_master = sql_master + "ORDER BY a.QUESTION_NO";
		sql_qry_grcode = "SELECT  ";
		sql_qry_grcode = sql_qry_grcode + "SEARCHGRCODENM('" + p_grcode_Param + "', '" + p_grseq_Param + "')  ";
		sql_qry_grcode = sql_qry_grcode + "|| ' ' || TO_NUMBER(SUBSTR(GRSEQ,5,2)) || '기' AS GRCODENM ";
		sql_qry_grcode = sql_qry_grcode + "FROM TB_GRSEQ ";
		sql_qry_grcode = sql_qry_grcode + "WHERE GRCODE = '" + p_grcode_Param + "' AND GRSEQ = '" + p_grseq_Param + "'";
	}

	private void initSQLdetailVarient(){
		sql_detail = "SELECT a.question_no, a.question as question, searchsubjnm(a.tsubj) as subjnm, to_char(b.answer_no) || '.' as answer_no, ";
		sql_detail = sql_detail + "b.answer as answer, b.answer_kind as answer_kind  ";
		sql_detail = sql_detail + "FROM  TB_GRINQ_QUESTION_SET a, TB_GRINQ_SAMP_SET b ";
		sql_detail = sql_detail + "WHERE a.question_checked_no = 0   ";
		sql_detail = sql_detail + "AND a.samp_checked_no = 0  AND a.title_no=b.title_no AND a.question_no=b.question_no ";
		sql_detail = sql_detail + "AND a.set_no=b.set_no ";
		sql_detail = sql_detail + "and a.title_no = " + p_title_no_Param + " AND a.set_no = " + p_set_no_Param + " ";
		sql_detail = sql_detail + "AND a.QUESTION_NO = " + master_QUESTION_NO + "  ";
		sql_detail = sql_detail + "ORDER BY a.QUESTION_NO,b.answer_no";
	}

	//-----ResultSet 객체에서 필드 값 설정 함수-----
	//ResultSet master 객체에서 필드 값 설정 함수
	private void setFieldVariableFrommaster() throws Exception{
		master_QUESTION_NO=GetB(master.getString(1));
	...
	private void setFieldVariableFromdetail() throws Exception{
		...
		detail_ANSWER_NO=GetB(detail.getString(4));
	...
	//ResultSet qry_grcode 객체에서 필드 값 설정 함수
	private void setFieldVariableFromqry_grcode() throws Exception{
		qry_grcode_GRCODENM=GetB(qry_grcode.getString(1));
	}

	//ResultSet qry_grcode 객체의 필드 값 초기화 함수
	private void setFieldInitFromqry_grcode(){
		qry_grcode_GRCODENM="";
	}

...
<%
	...
	String textAreaStr = "{name}님 {grname}과정 학습진도율이 부족합니다.분발 바랍니다";
	String tmpStr = "";
	...
	for(int i=0; i < listMap.keySize("userno"); i++){
		...
		//checkBox
		tmpStr = "<input type='checkbox' class='chk_01' name='userno[]' value=\"" + listMap.getString("grcodeUserno", i) + "\" " + tmpDisableStr + "> "+ listMap.getString("name", i);
		listStr += "\n	<td>" + tmpStr + "</td>";
...
<input type="hidden" name="qu"					value='cyber_sms'>
...
<input type="hidden" name="searchStartpt"		value='<%=requestMap.getString("searchStartpt")%>'>
<input type="hidden" name="searchEndpt"			value='<%=requestMap.getString("searchEndpt")%>'>

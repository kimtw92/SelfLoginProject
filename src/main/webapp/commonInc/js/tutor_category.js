...
function fnGoTimeTableByAdmin(){
	location.href = "/courseMgr/timeTable.do?mode=list&menuId=2-1-2";
}

// 과정기수관리(시스템관리자)
function fnGoCourseSeqByAdmin(){
	location.href = "/courseMgr/courseSeq.do?mode=list&menuId=2-1-1";
}
...
function fnGoTutorSetting(){
	location.href = "/tutorMgr/tclass.do?mode=list&menuId=4-1-2";
}

// 과목별학습현황(시스템 관리자)
function fnGoStudyStaSubjByAdmin(){
	location.href = "/courseMgr/studySta.do?mode=subj_list&menuId=2-3-1";
}
// 전체학습현황(시스템 관리자)
function fnGoStudyStaTotalByAdmin(){
	location.href = "/courseMgr/studySta.do?mode=total_list&menuId=2-3-2";
}
// 사이버학습현황(시스템 관리자)
function fnGoStudyStaCyberByAdmin(){
	location.href = "/courseMgr/studySta.do?mode=cyber_list&menuId=1-2-1";
}
// 혼합교육학습현황(시스템 관리자)

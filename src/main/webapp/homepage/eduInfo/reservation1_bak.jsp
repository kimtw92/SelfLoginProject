...
<%@ page import="loti.courseMgr.service.ReservationSV "%>
...
<%
	...
	int year		= Integer.parseInt(sv.getYear());
	int month		= (Integer.parseInt(sv.getMonth())-1);
	...
	int today = (Integer.parseInt(sv.getDay()));
	...
	if("6".equals(place) || "7".equals(place)) {
		for(int i=1; i < 32 ; i++) {
            if(p_yyyy.equals(String.valueOf(year)) && p_mm.equals(String.valueOf((month+1))) && p_day.indexOf(String.valueOf(","+i+",")) != -1)  {
			    allDays[i]="<font class=\"green\">예약완료</font>";
            } else{
		...
		if(start == 1) {
			...
			for(int i=1; i < 32 ; i+=7) {
                if(p_yyyy.equals(String.valueOf(year)) && p_mm.equals(String.valueOf((month+1))) && p_day.indexOf(String.valueOf(","+i+",")) != -1)  {
                    daysAm[i]="<font class=\"green\">예약완료</font>";
                    daysPm[i]="<font class=\"green\">예약완료</font>";
                    allDays[i]="<font class=\"green\">예약완료</font>";                
                } else {
		...
		//1일이 일요일이 아닌 경우
			...
			for(int i=sat; i < 32; i+=7) {
                ...
                if(p_yyyy.equals(String.valueOf(year)) && p_mm.equals(String.valueOf((month+1))) && p_day.indexOf(String.valueOf(","+(i+1)+",")) != -1)  {
                    daysAm[i+1]="<font class=\"green\">예약완료</font>";
                    daysPm[i+1]="<font class=\"green\">예약완료</font>";
                    allDays[i+1]="<font class=\"green\">예약완료</font>";   
                } else {
                    //일요일 설정
	...
	if("6".equals(place) || "7".equals(place)) {
		...
		for(int i = 0 ; i < holydaylistMap.keySize(); i++) {	
            if(p_yyyy.equals(String.valueOf(year)) && p_mm.equals(String.valueOf((month+1))) && p_day.indexOf(String.valueOf(","+Integer.parseInt(holydaylistMap.getString("dd", i))+",")) != -1)  {
                daysAm[Integer.parseInt(holydaylistMap.getString("dd", i))]  =  "<font class=\"green\">예약완료</font>";
                daysPm[Integer.parseInt(holydaylistMap.getString("dd", i))]  ="<font class=\"green\">예약완료</font>";
                allDays[Integer.parseInt(holydaylistMap.getString("dd", i))] ="<font class=\"green\">예약완료</font>";
            } else {
	...
	for(int i = 0 ; i < listMap.keySize("taPk"); i++) {
		...
		if((month+1) == rentMonth) {
			...
			if(time.equals("am")) {
				daysAm[index] ="<font class=\"green\">예약완료</font>";
			} else if(time.equals("pm")) {
				daysPm[index] ="<font class=\"green\">예약완료</font>";
			} else if(time.equals("all")) {
				if("6".equals(place) || "7".equals(place)) {
				} else {
					allDays[index] ="<font class=\"green\">예약완료</font>";
				}
			}
		}
	}
   ...
   //오전오후예약시. 종일 예약 불가
   for(int i=0; i<=31; i++) {
	   if(daysAm[i].equals("<font class=\"green\">예약완료</font>") ) {
		   allDays[i] = "<font class=\"gray\">예약불가</font>";
	   } 
	   if(daysPm[i].equals("<font class=\"green\">예약완료</font>") ) {
		   allDays[i] = "<font class=\"gray\">예약불가</font>";
	   }	   
   }
   ...
   //종일예약시. 오전 오후 예약완료
   for(int i=0; i<=31; i++) {
	   if(allDays[i].equals("<font class=\"green\">예약완료</font>")) {
            if("6".equals(place) || "7".equals(place)) {
            } else {
               daysAm[i] = "<font class=\"gray\">예약불가</font>";
               daysPm[i] = "<font class=\"gray\">예약불가</font>";
            }
	   } 
   }

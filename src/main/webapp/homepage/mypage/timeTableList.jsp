...
<%
	...
	for(int i=0; i < listMap.keySize("grcode"); i++){
		count ++;
		out.print(listMap.getString("grcodeniknm", i) + " " + StringReplace.subString(listMap.getString("grseq", i), 4, 6) + "기");
		out.print(listMap.getString("classroom_name", i));
...
<script type="text/javascript" language="javascript" src="/AIViewer/AIScript.js"></script>
...
function go_list(){
	...
	pform.action = "/mypage/timeReport.do";
	pform.submit();

}

/*
1) grcode1=0020000008
2) grseq1=200401
3) p_week1=1
4) p_type1=1
5) p_limit1=1
6) p_tmtitle1=행정실무과정 01기
7) p_clroom1=반명
*/
function goPrint(){
	cgi_url = "http://152.99.42.130/report/report_19.jsp?p_grcode1="+$("grcode").value+"&p_grseq1="+$("grseq").value+"&p_week1="+$("studyWeek").value;
	cgi_url += "&p_type1=S&p_limit1=&p_tmtitle1="+$("subjnm").value+"&p_clroom1=";
	popAI(cgi_url);
}
...
<input type="hidden" name="subjnm"				value="<%=requestMap.getString("subjnm")%>">

<input type="hidden" name="studydate"			value="">
<input type="hidden" name="studytime"			value="">
<input type="hidden" name="subj"				value="">
			...
			<table cellspacing="0" cellpadding="0" border="0" width="100%" class="contentsTable">
				<tr>
					<td>
						...
						<!-- //[e] 상단 추가, 새로고침 버튼  -->
						<font color="blue" size="3" >※ 9시까지 등록 및 입실을 완료해주십시오.</font>
						<br />
						<!--[s] 리스트  -->
						<table class="datah01">
							<thead>
							<tr>
								...
								<th class="br0">금<%= dayComaArr[4] %></th>
							</tr>
							</thead>
			...
			<!--[e] Contents Form  -->
			<div class="btnC" style="width:550px;"><a href="javascript:goPrint();" style="line-height:180%"><img src="/images/<%=skinDir %>/button/btn_print.gif"></a></div>

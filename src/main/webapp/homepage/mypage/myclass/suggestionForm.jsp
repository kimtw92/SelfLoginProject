...
function go_add(){
	...
	if(confirm("등록 하시겠습니까?")){
		...
		$("mode").value = "suggestionExec";
		$("qu").value = "insertSuggestion";
		tx_editor_form.action = "/mypage/myclass.do?mode=suggestionExec";
...
function go_save(qu){
	...
	<%if(requestMap.getString("qu").equals("insertSuggestion") || requestMap.getString("qu").equals("insertReplySuggestion") ){%>
	if(confirm("등록 하시겠습니까?")){
...
//리스트
function go_list(){
	$("mode").value = "suggestionList";
	tx_editor_form.action = "/mypage/myclass.do?mode=suggestionList";
	tx_editor_form.submit();
}
...
onload = function()	{
	$("title").focus();
	<%= fileStr %>
	multi_selector.limitFile = ["exe", "php", "html", "jsp"];
}
...
<input type="hidden" name="INNO_SAVE_DIR"		value='<%=Constants.UPLOADDIR_HOMEPAGE%>'>
					<div id="contents">
						...
						<table class="bView01">
						<tr>
							<th class="bl0"  width="100"><img src="/images/<%= skinDir %>/table/th_sbj.gif" alt="제목" /></th>
							<td colspan="3">

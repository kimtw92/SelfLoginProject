...
if (Util.getIntValue(viewMap.getString("groupfileNo"), 0) > 0){
	...
	if (fileGroup.keySize("groupfileNo") > 0){
		...
		for(int i=0,l=fileGroup.keySize("groupfileNo");i<l;i++){
			...
			innerHtml.append("<a href=\"javascript:fileDownload('"+fileGroup.getString("groupfileNo", i)+"', '"+fileGroup.getString("fileNo", i)+"');\">");
			//innerHtml.append("<a href=\"javascript:fileDownloadPopup('"+fileGroup.getString("groupfileNo", i)+"');\">");
...
function setReWrite(){
	$("qu").value = "insertReplySuggestion";
	pform.action = "/mypage/myclass.do?mode=suggestionWrite";
	pform.submit();
}

function setDelete(){
	$("qu").value = "deleteSuggestion";
	pform.action = "/mypage/myclass.do?mode=suggestionExec";
	pform.submit();
}

function setModify(){
	$("qu").value = "modifySuggestion";
	pform.action = "/mypage/myclass.do?mode=suggestionModify";
	pform.submit();
}
//-->
</script>
          ...
          <div class="sub_visual"><img src="/homepage_new/images/common/sub_visual1.jpg" alt="인재원 소개 인천광역시 인재개발원에 대한 소개 및 시설정보를 제공합니다."></div>
            ...
            <div class="mytab01">
              <ul>
                ...
                <li><a href="javascript:fnGoMenu('1','suggestionList')""><img src="/homepage_new/images/M1/tab06_on.gif" alt="과정질문방"/></a></li>
...
<input type="hidden" name="currPage"	value="<%= requestMap.getString("currPage")%>">
				<div id="contents">
						...
						<!-- title --> 
						<h4 class="h4Ltxt"><%=viewMap.getString("grcodeniknm") %></h4>
						<!-- //title -->
						<div class="h9"></div>
						...
						<tr>
							...
							<td width="320"><%=viewMap.getString("name") %></td>
							<th width="75"><img src="/images/<%= skinDir %>/table/th_date.gif" alt="작성일" /></th>
							<td width="100"><%=viewMap.getString("regdate") %></td>
						</tr>
						<tr>
							<th class="bl0" width="75">분류</th>
							<td width="320"><%=viewMap.getString("category") %></td>
							<th width="75">답변</th>
							<td width="100"><%=viewMap.getString("finishYn") %></td>
						</tr>
						<tr bgcolor="#FFFFFF">
							<th class="bl0" width="75">첨부파일</th>
							<td colspan="3"> <%= innerHtml.toString() %> </td>
						</tr>
						<tr>
							<td class="bl0 cont" colspan="4">
							<%=StringReplace.convertHtmlDecodeNamo(viewMap.getString("content"))%>
							</td>
						</tr>
						</table>	
						<!-- //view -->

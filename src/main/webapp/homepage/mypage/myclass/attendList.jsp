...
if(listMap.keySize("grcode") > 0) {
	...
	*/
	...
	for(int i=0; i < listMap.keySize("grcode"); i++){
		...
		if (checkValue_social.indexOf(listMap.getString("grcode", i)) != -1) {
			if(!"10C0000055".equals(listMap.getString("grcode", i)) && !"10C0000056".equals(listMap.getString("grcode", i))) {
				...
				if (listMap.getString("userno",i).length() > 0){
					...
					if("10G0000243".equals(listMap.getString("grcode", i))){
						sbListHtml.append("	<td><a href=\"javascript:go_apply('"+listMap.getString("grcode",i)+"','"+listMap.getString("grseq",i)+"','"+requestMap.getString("userno")+"','"+listMap.getString("grgubun",i)+"')\"><img src=\"/images/"+skinDir+"/button/btn_request03.gif\" alt=\"취소\" /></td>\n");
					} else if (listMap.getString("restrict").length() > 0){
						sbListHtml.append("	<td><a href=\"javascript:alert('귀하는 이전기수 사이버교육을 미수료하여 60일간 수강신청이 제한되어 있습니다.');\"><img src=\"/images/"+skinDir+"/button/btn_request03.gif\" alt=\"취소\" /></a></td>\n");
					} else {
				...
				if (listMap.getString("userno",i).length() > 0){
					...
					if (listMap.getString("restrict").length() > 0){
						...
						if ( (Integer.parseInt(seated)>= Integer.parseInt(listMap.getString("tseat",i)))) {
							...
							sbListHtml2.append("	<td><a href=\"javascript:go_apply('"+listMap.getString("grcode",i)+"','"+listMap.getString("grseq",i)+"','"+requestMap.getString("userno")+"','"+listMap.getString("grgubun",i)+"')\"><img src=\"/images/"+skinDir+"/button/btn_request03.gif\" alt=\"취소\" /></td>\n");
						}
			...
			if(!"10C0000055".equals(listMap.getString("grcode", i)) && !"10C0000056".equals(listMap.getString("grcode", i))) {
				...
				if (listMap.getString("userno",i).length() > 0){
					...
					if (listMap.getString("restrict").length() > 0){
						...
						// sbListHtml.append("<td><input type=\"checkbox\" name=\"checkBox\" value=\""+listMap.getString("grcode",i)+"|"+listMap.getString("grseq",i)+"|"+listMap.getString("grcodeniknm",i)+"\"></td>");
						sbListHtml3.append("	<td><a href=\"javascript:go_apply('"+listMap.getString("grcode",i)+"','"+listMap.getString("grseq",i)+"','"+requestMap.getString("userno")+"','"+listMap.getString("grgubun",i)+"')\"><img src=\"/images/"+skinDir+"/button/btn_request03.gif\" alt=\"취소\" /></td>\n");
        ...
        <div id="contnets_area">
          <div class="sub_visual1">마이페이지</div>
            <div style="margin-bottom: -20px;" class="local">
              <h2>교육신청 및 취소</h2>
                <div class="navi"><img src="/homepage_new/images/common/icon_home.gif" alt="home"> &gt; 마이페이지 &gt; 교육신청 및 취소 &gt; <span>교육신청</span></div>
            </div>
			<div class="contnets">
            <!-- contnet -->
...
<br/><br/>
           <ol class="TabSub">
            <li class="TabOn"><a href="javascript:fnGoMenu('1','attendList')">교육신청</a></li>
            <li class="last"><a href="javascript:fnGoMenu('1','attendDetail')">교육신청이력</a></li>
          </ol>

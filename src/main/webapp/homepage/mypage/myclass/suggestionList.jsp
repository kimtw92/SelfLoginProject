...
if(listMap.keySize("no") > 0){		
	for(int i=0; i < listMap.keySize("no"); i++){
		...
		sbListHtml.append("<td class=\"bl0\">"+((pageNavi.getTotalCnt() - requestMap.getInt("rowSize") * (requestMap.getInt("currPage") - 1) - i))+"</td>\n");
				
		//답변의 Depth 만큼 들여쓰기
		String depthSpace = "";
		String replyIcon = "";
		if(listMap.getInt("depth", i) > 0) {
			for(int ii=0; ii < listMap.getInt("depth", i); ii++) {
				depthSpace = "&nbsp;&nbsp;";
			}
			replyIcon = depthSpace + "<img src=\"/images/"+skinDir+"/icon/icon_re.gif\" alt=\"\" />";
		}
		
		sbListHtml.append("<td class=\"sbj\">"+replyIcon+" <a href=\"javascript:onView('"+listMap.getString("no",i)+"');\">"+StringReplace.subStringPoint(listMap.getString("title",i),23)+"</a></td>\n");
		sbListHtml.append("<td>"+listMap.getString("category",i)+"</td>\n");
		sbListHtml.append("<td>"+listMap.getString("finishYn",i)+"</td>\n");
	...
	sbListHtml.append("<tr>");
	sbListHtml.append("<td colspan=\"6\">등록된 질문이 없습니다!");
...
// 리스트
function fnList(){
	pform.action = "/mypage/myclass.do?mode=suggestionList";
	pform.submit();
}
//리스트
function goSearch(){
...
function onView(form){
	$("qu").value = "suggestionView";
	pform.action = "/mypage/myclass.do?mode=suggestionView&seq="+form;
	pform.submit();
}
//글쓰기
function goWrite(){
	$("qu").value = "insertSuggestion";
	pform.action = "/mypage/myclass.do?mode=suggestionWrite";
	pform.submit();
}
//-->
</script>
          ...
          <div class="sub_visual"><img src="/homepage_new/images/common/sub_visual1.jpg" alt="인재원 소개 인천광역시 인재개발원에 대한 소개 및 시설정보를 제공합니다."></div>
            ...
            <div class="mytab01">
              <ul>
                ...
                <li><a href="/mypage/myclass.do?mode=suggestionList&type=<%=type%>"><img src="/homepage_new/images/M1/tab06_on.gif" alt="과정질문방"/></a></li>
...
<div id="content">
						...
						<!-- title --> 
						<h4 class="h4Ltxt"><%=listMap.getString("grcodeniknm") %></h4>
						<!-- //title -->
						<div class="h9"></div>
						...
						<tr>
							...
							<th><img src="/images/<%= skinDir %>/table/th_div.gif" alt="분류" /></th>
							<th><img src="/images/<%= skinDir %>/table/th_answr.gif" alt="답변" /></th>

...
public class NamoMime
{
	...
	*/
		...
		while(((line = br.readLine()) != null) && (nowState != MIME.END))							// 한줄씩 읽어 들인다
		{
			...
			else
			{
				...
				if(nowState == MIME.HEADER)
				{
					...
					{
						try{
							compare = line.substring(line.indexOf("&lt;") + 4, line.indexOf("&gt;"));
						}catch(Exception e){
	 ...
	 */
	public String changeCIDPath(String content)
	{
	...
	public void saveFile() throws MimeDecodeException
	{
		...
		for(i = 1; i < decodePart.size(); i++)
		{
			...
			try
			{
				...
				outFile = new File(savePath + "/" + fileName);	 					// 파일을 새로 생성한다 (2008.06.09) 이용문 수정.
	...
	{
		//
		//2008.06.09 이용문 수정. (파일 등록시 파일 이름을 현재 날짜시간 으로 변경하여 저장 하기 위해.)
		/*
		...
		*/
		...
		String writableName = DateUtil.getDateTimeMinSec() + ext;
		...
		while(writeFile.exists() == true)
		{
			writableName = DateUtil.getDateTimeMinSec() + "[" + Integer.toString(i) + "]" + ext;
			writeFile = null;
			writeFile = new File(savePath + "/" + writableName);
			i++;
		}

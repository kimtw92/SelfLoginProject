 ...
 * @version	2005. 7. 19.
 */
public class FrontPrevNextRenderer {

	private String prevNextBox;

	public FrontPrevNextRenderer(DataMap prevNextData){
		doRender(prevNextData, "list_cmt");
	}

	public FrontPrevNextRenderer(DataMap prevNextData, String linkStyle){
		doRender(prevNextData, linkStyle);
	}
    ...
    public void doRender(DataMap prevNextData, String linkStyle) {
		...
		for (int i=0; i<prevNextData.keySize(); i++){
			if(prevNextData.getString("sep",i).equals("PREV")){
				prev = "<a href=\"javascript:goDetail('" + prevNextData.getString("nstseq",i) + "')\" class='" + linkStyle + "'>" + Util.cutString(prevNextData.getString("mtitle",i),50) + "</a>";
			}else{
				next = "<a href=\"javascript:goDetail('" + prevNextData.getString("nstseq",i) + "')\" class='" + linkStyle + "'>" + Util.cutString(prevNextData.getString("mtitle",i),50) + "</a>";
			}
		}

 ...
 * @version 2005. 7. 02.
 */
public class FrontFileListRenderer
{
     ...
     */
    public FrontFileListRenderer (DataMap fileData, int maxUploadFileCnt)
	{
        super();
     ...
     */
    public FrontFileListRenderer (DataMap fileData, int maxUploadFileCnt, String gubun, String htmlNameAttr, String formCheckFuncName)
	{
        super();
		this.fileData = fileData;
        this.maxUploadFileCnt = maxUploadFileCnt;
     ...
     * @param width
     */
    public FrontFileListRenderer (DataMap fileData, int maxUploadFileCnt, String gubun, String htmlNameAttr, String formCheckFuncName, String width)
	{
        super();
		this.fileData = fileData;
        this.maxUploadFileCnt = maxUploadFileCnt;
     ...
     */
    public FrontFileListRenderer (DataMap fileData, int maxUploadFileCnt, String gubun, String htmlNameAttr, String formCheckFuncName, String width, String cssName)
	{
        super();
		this.fileData = fileData;
        this.maxUploadFileCnt = maxUploadFileCnt;
    ...
    public void setAttachFileCnt(DataMap fileData)
	{
		...
		else
		{
		   ...
		   {
              if(fileData.getString("cfgubn", i).equals(this.gubun))
			  {
					size++;
              }
           }
	...
	public String attachFileCntRender()
	{
		...
		tagBuffer.append("<select name=\"attachFileCnt\" style=\"width:66;\" onChange=\"setAttachFileCnt('"+htmlNameAttr+"', this.value)\">\r\n");
	...
	public String fileBoxRender()
	{
        ...
        for (int i = 0 ; i < fileData.keySize() ; i++)
		{
            ...
            if ( (flag ? true : fileData.getString("cfgubn", i).equals(this.gubun)) )
			{
	            tagBuffer.append("&nbsp;").append(fileData.getString("mrname", i));
	            tagBuffer.append("&nbsp;(").append(fileData.getString("afsize", i)).append("바이트)<br>");
	            ...
	            tagBuffer.append("<input type=\"file\" name=\"").append(htmlNameAttr);
				...
				tagBuffer.append("\" value=\"").append(fileData.getInt("nstseq", i)).append("\">\r\n");
				k++;
			}
		...
		if (maxUploadFileCnt > 1)
		{
			...
			{
				tagBuffer1.append("<input type=\"file\" name=\"").append(htmlNameAttr).append("\" class=\"" + this.cssName + "\" style=\"width:" + this.width + ";" );
				if (i != 0)
				{
				    dispOption = "display:none;";
				}
				tagBuffer1.append(dispOption).append("\" "+ checkStr +"  >\r\n");
			}
		}
		else
		{
			...
			{
		        tagBuffer1.append("<input type=\"file\" name=\"").append(htmlNameAttr).append("\"  class=\"" + this.cssName + "\" style=\"width:" + this.width + ";\"  "+ checkStr +" >\r\n");
		    }
		}
    ...
    public void setChangeFileCntScript()
	{
		...
		tagBuffer.append("	var frm = document.f;\r\n");
		tagBuffer.append("	for (var i = 0; i < parseInt(val); i++) {\r\n");
		tagBuffer.append("		frm[name][").append("i].style.display = \"\";\r\n");
        tagBuffer.append("	}\n");
        tagBuffer.append("	for (var j = parseInt(val); j < "+maxUploadFileCnt+"; j++) {\r\n");
        tagBuffer.append("		frm[name][").append("j].style.display = \"none\";\r\n");
        tagBuffer.append("	}");
        tagBuffer.append("}");
	...
	public void setFileFormCheckScript()
	{
        ...
        if (maxUploadFileCnt > 1)
		{
			...
			{
				tagBuffer1.append("		if ((document.f[\"del" + gubun +"Check\"])&&(document.f[\"del").append(gubun).append("Check\"].checked)) {\r\n");
				tagBuffer1.append("			document.f.oldAttachFileSeq.value = document.f.oldAttachFileSeq.value + document.f[\"old").append((htmlNameAttr.substring(0,1)).toUpperCase()).append(htmlNameAttr.substring(1)).append("Seq").append("\"].value + \",\";\r\n");
			...
			{
				tagBuffer1.append("		if ((document.f[\"del" + gubun +"Check\"][i])&&(document.f[\"del").append(gubun).append("Check\"][i].checked)) {\r\n");
				tagBuffer1.append("			document.f.oldAttachFileSeq.value = document.f.oldAttachFileSeq.value + document.f[\"old").append((htmlNameAttr.substring(0,1)).toUpperCase()).append(htmlNameAttr.substring(1)).append("Seq").append("\"][i].value + \",\";\r\n");
				...
				tagBuffer1.append("				alert(\"첨부파일을 수정하시려면 체크박스를 체크하셔야 합니다.\");\r\n");
				tagBuffer1.append("				document.f[\"").append("del").append(gubun).append("Check\"][i].focus();\r\n");
				tagBuffer1.append("				return false;\r\n");
		    	tagBuffer1.append("			}\r\n");
			}
    ...
    public String toHtml()
	{
        ...
        if (maxUploadFileCnt > 1)
		{
            html = attachFileCntRender()  + fileBoxRender();
        }
		else
		{
            html = fileBoxRender();
